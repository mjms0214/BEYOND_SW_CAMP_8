...
public class ValueController {
	 ...
	 * 	- @ResponseBody 어노테이션이 붙은 메소드의 반환형이 String일 경우, 
	 *    해당 요청을 보낸 클라이언트에 전달할 데이터를 의미한다. 
	 *    
	 * Jackson 라이브러리
	 * 	- 자바 객체를 JSON 형태의 데이터로 변환하기 위한 라이브러리이다.
	 * 	- 스프링에서는 jackson 라이브러리를 추가하고 @ResponseBody를 사용하면 
	 *    컨트롤러 메소드에서 리턴되는 객체를 자동으로 JSON 문자열로 변경해서 응답한다.
	 *  
	 * @RestController
	 * 	- @Controller과 @ResponseBody를 합쳐 놓은 역할을 수행한다.
	 * 	- 해당 어노테이션이 붙어 있는 컨트롤러의 모든 메소드에서 데이터를 반환하는 경우에 사용한다.
	 */
